...
2. **个人记忆** → 由 MCP memory（`@modelcontextprotocol/server-memory`）或 claude-mem 自动管理，**禁止**手动 Write `memory/*.md`
...
个人记忆通过 **MCP memory**（`@modelcontextprotocol/server-memory`）或 **claude-mem** 自动管理，**禁止**手动写入本地 `memory/*.md` 文件。
