---
name: "source-command-pr-all"
description: "先 /pull-all 同步默认分支并按项目约定解决冲突，再提交子模块 + 主仓库 gh pr create 到默认分支"
---

# source-command-pr-all

Use this skill when the user asks to run the migrated source command `pr-all`.
...
按项目约定的冲突处理原则处理（见 `AGENTS.md`「合并冲突处理原则」与 `/merge-conflict`）：
